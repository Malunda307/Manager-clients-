...
function rcm(){
  ...
  g.innerHTML = prods.map(function(p){
    ...
    var q = inc ? inc.qty : 0;
    return '<div class="pc"><div class="pi">' + (p.emoji || '🍽️') + '</div><div class="pn"><div class="n">' + escHtml(p.name) + '</div><div class="d">' + escHtml(p.category) + '</div><div class="pr">' + fmt(p.price) + '</div><div class="pa"><button class="qb" onclick="ucc(\'' + p.id + '\',-1)">−</button><span class="qv">' + q + '</span><button class="qb" onclick="ucc(\'' + p.id + '\',1)">+</button></div></div></div>';
  }).join('');
}
...
function rcc(){
  ...
  c.innerHTML = ccart.map(function(it){
    ...
    return '<div class="ci"><div class="cii">' + (em ? em.emoji : '🍽️') + '</div><div class="cin"><div class="nn">' + escHtml(it.name) + '</div><div class="pp">' + fmt(it.price) + ' x ' + it.qty + ' = ' + fmt(it.price * it.qty) + '</div></div><div class="pa"><button class="qb" onclick="ucc(\'' + it.id + '\',-1)">−</button><span class="qv">' + it.qty + '</span><button class="qb" onclick="ucc(\'' + it.id + '\',1)">+</button></div></div>';
  }).join('');
...
function os(){ cm('mos'); t('Scanner QR non configure'); }
...
function hp(e){
  ...
  if(id){
    ...
    if(p){p.name=name;p.category=cat;p.price=price;p.cost=cost;p.available=av;}
  } else {
    db.products.push({id:uid(),name:name,category:cat,price:price,cost:cost,available:av,emoji:ge(cat)});
  }
  sdb(); rml(); t('Produit sauvegarde'); rpf();
}

function rpf(){ document.getElementById('pf').reset(); document.getElementById('pid').value=''; document.getElementById('pca').style.display='none'; }
...
function hsm(e){
  ...
  if(type==='in') s.qty+=q; else s.qty-=q;
...
function hse(e){
  ...
  db.config.defaultCom=parseFloat(document.getElementById('cfg-co').value)||500;
  db.config.reinvestRate=parseFloat(document.getElementById('cfg-r').value)||30;
  db.config.goalOrders=parseFloat(document.getElementById('cfg-go').value)||500;
  db.config.goalRevenue=parseFloat(document.getElementById('cfg-gr').value)||1500000;
...
function rml(filter){
  ...
  list.forEach(function(p){
    ...
    tbody.innerHTML+='<tr><td>'+escHtml(p.name)+'</td><td>'+p.category+'</td><td>'+fmt(p.price)+'</td><td>'+fmt(p.cost)+'</td><td>'+fmt(p.price-p.cost)+'</td><td>'+st+'</td><td><button class="bs bs2" onclick="ep(\''+p.id+'\')">✎</button> <button class="be bs2" onclick="dp(\''+p.id+'\')">✕</button></td></tr>';
  });
}
function ep(id){
